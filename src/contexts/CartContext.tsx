  ...
  const updateItem = (product: IProduct, operand: TOperand, count = 1) => {
    setCart((c) => {
      ...
      if (item) {
        ...
        const newQauntity =
          operand === "+" ? item.quantity + count : Math.max(item.quantity - count, 0);
        ...
        // If the new quantity is 0 or less, remove it from the cart, else update cartItem
        if (newQauntity <= 0) {
          arr = c.items.filter((prevItem) => prevItem.item.id !== item.item.id);
        } else {
          // Get a new array with an updated cartItem at the specified index
          const index = c.items.indexOf(item);
          arr = c.items.map((item, i) => (i === index ? { ...item, quantity: newQauntity } : item));
        }
